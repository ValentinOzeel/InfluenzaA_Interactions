...
foreach my $flu (@Flu) {
   ...
   unless ($flu eq "FluB" and $state eq "Intermediates" or $flu eq "FluB" and $state eq "Postfusion"){
    ...
    while (keys(%$copy_H_category)) {
      ...
      LOOP:foreach $_ (@$ref_array_v) {
          ...
          my $In_path = "$path_environment/Interactions/Result/$flu/$state/$subtype/$PDB_code/HashCountInteractions$PDB_code.dat";
  ...
  my $HandleJson_normal = "$path_environment/Interactions/Result/Count_normal";
  my $HandleJson_proportionvstotal = "$path_environment/Interactions/Result/Count_proportionvstotal";
  my $HandleJson_proportionvstotaltype = "$path_environment/Interactions/Result/Count_proportionvstotaltype";
...
###################################### SUBS ################################################
       ...
       sub save_json {
             ...
             my $JSON_H = encode_json($H_ref);   print HFILE $JSON_H;
             close HFILE;
